...
function () {
  ...
  _createClass(PrioritizedTaskExecutor, [{
    ...
    value: function execute(priority, task) {
      ...
      if (this.currentPoolSize < this.maxPoolSize) {
        ...
        task(function () {
          ...
          if (_this.queue.length > 0) {
            // KJ: RESEARCH - removed comparator, it was slow to call it for every task
        ...
        // KJ: RESEARCH - sort only when adding
        addAndSort2(this.queue, {
          priority: priority,
          task: task
        });
        // this.queue.push({
      ...
      // KJ: RESEARCH -- fast insert sort
      function addAndSort2(arr, val) {
        arr.push(val);
        let i = arr.length - 1;
        let item = arr[i];
        while (i > 0 && item.priority > arr[i-1].priority) {
          arr[i] = arr[i-1];
          i -= 1;
        }
        arr[i] = item;
