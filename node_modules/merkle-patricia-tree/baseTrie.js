...
function () {
  ...
  _createClass(Trie, [{
    ...
    value: function _findValueNodes(onFound, cb) {
      this._walkTrie(this.root, function (nodeRef, node, key, depth, walkController) {  // KJ: RESEARCH - added depth
        var fullKey = key;
        ...
        if (node.type === 'leaf') {
          // found leaf node!
          onFound(nodeRef, node, fullKey, depth, walkController.next);  // KJ: RESEARCH - added depth
        } else if (node.type === 'branch' && node.value) {
          // found branch with value
          onFound(nodeRef, node, fullKey, depth, walkController.next);  // KJ: RESEARCH - added depth
        } else {
          // keep looking for value nodes
          //walkController.next();  // KJ: RESEARCH - this line commented out
          onFound(nodeRef, node, fullKey, depth, walkController.next); //   KJ: RESEARCH - this line added
        }
      }, cb);
    }
    /*
    ...
    value: function _walkTrie(root, onNode, onDone) {
      ...
      this._lookupNode(root, function (node) {
        // KJ: RESEARCH - added depth
        processNode(root, node, null, 0, function (err) {
          if (err) {
            return onDone(err);
          }
      ...
      function processNode(nodeRef, node, key, depth, cb) {  // KJ: RESEARCH - added depth
        if (!node || aborted) {
          return cb();
        }
        ...
        var walkController = {
          ...
          next: function next(depth) { // KJ: RESEARCH - added depth
            if (aborted || stopped) {
              return cb();
            }

            var children = node.getChildren();
            // KJ: RESEARCH - this added: attempt to fix out-of-memory
            // console.log("OoM fix applied")
            ...
            //     processNode(childRef, childNode, childKey, depth + 1, cb); // KJ: RESEARCH - added depth
            //   });
            // });

            // KJ: RESEARCH - below removed: attempt to fix out-of-memory
          ...
          only: function only(childIndex) {
            ...
            // KJ: RESEARCH - removed async to prevent JavaScript heap out of memory
            // console.log("OoM fix applied")
            // self._lookupNode(childRef, function (childNode) {
